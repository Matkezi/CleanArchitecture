export const API = {
  ACCOUNT: {
    ...
    PASSWORD_RESET_REQUEST: (email: string): string =>
      `Account/password-reset-email/${email}`,
    RESET_PASSWORD: (email: string, password: string, token: string): string =>
      `Account/password-reset/${email}/${token}/${password}`,
    CHANGE_EMAIL_REQUEST: (email: string, newEmail: string): string =>
      `Account/email-reset-email/${email}/${newEmail}`,
    CHANGE_EMAIL: (email: string, newEmail: string, token: string): string =>
      `Account/change-email/${email}/${newEmail}/${token}`,
    CHANGE_PASSWORD: (
      email: string,
      currentPassword: string,
      newPassword: string
    ): string =>
      `Account/change-password/${email}/${currentPassword}/${newPassword}`,
  },
  CHARTER: {
  ...
  SKIPPER: {
    ...
    BOOKING: {
      ...
      ACCEPT: (id: number): string => `SkipperBooking/accept/${id}`,
      DECLINE: (id: number): string => `SkipperBooking/decline/${id}`,
    },
    PROFILE: {
