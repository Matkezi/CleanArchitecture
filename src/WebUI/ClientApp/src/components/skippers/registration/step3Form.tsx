...
export const SkipperRegistrationFormStep3: React.FC<IProps> = (props) => {
  ...
  const handleChange = (event: any) => {
    if (event.target.files[0] && (event.target.files[0].type.endsWith("jpeg") || event.target.files[0].type.endsWith("png") || event.target.files[0].type.endsWith("jpeg"))) {
      setLicenceImage({
        data: event.target.files[0], url: URL.createObjectURL(event.target.files[0])
      })
  ...
  function getImageHolder() {
    if (licenceImage.url || (licencePDF.data && licencePDF.data.name)) {
      ...
      return <div className={styles.licenceFile}><span >Choose licence in .pdf or image format</span></div>
    }
  }
  ...
  return (
    <div>
      <span className={styles.headlineSmall} style={{ marginLeft: 0 }}>Upload your files.</span>
        ...
        <Grid container direction="row" spacing={4}>
          <Grid item xs={12} sm={6}>
            ...
            <Grid container item spacing={4}>
              ...
              <Grid xs={12} item container direction="row" justify="center">
                <Grid item>
                  <button type="button" className={styles.uploadButton} onClick={() => fileInputRef.click()}>
                    Choose file
                  </button>
                </Grid>
              </Grid>
          ...
          <Grid item xs={12} sm={6}>
            ...
            <Grid container spacing={4}>
              <Grid item xs={12} sm={6} className={styles.formSelect} container direction="row">
                <Grid item xs={12}>
                  <div className={styles.dateOfBirth}>
                    <DatePickerComponent
                      ...
                      change={(value: any) => handleDateChange(value.value, "DATE_OF_ISSUE")}
                      showTodayButton={false}
                      firstDayOfWeek={1}
                      format={"dd.MM.yyyy."}
                      value={dateOfIssue! === undefined ? undefined : new Date(dateOfIssue!)}
                    />
                  </div>
                </Grid>
              </Grid>
                ...
                <Grid item xs={12}>
                  <div className={styles.dateOfBirth}>
                    <DatePickerComponent
                      ...
                      id="datepicker"
                      className={styles.datePicker}
                      change={(value: any) => handleDateChange(value.value, "VALID_UNTIL_DATE")}
                      showTodayButton={false}
                      firstDayOfWeek={1}
                      format={"dd.MM.yyyy."}
                      value={validUntilDate! === undefined ? undefined : new Date(validUntilDate!)}
                    />
                  </div>
                </Grid>
              </Grid>
            ...
            <Grid container spacing={4}>
              <Grid item xs={12} sm={6} style={{ paddingTop: 0 }}>
                <FormControl className={styles.formSelect} style={{ marginTop: 30 }}>
                  <Select
                    isClearable={true}
                    isSearchable={false}
                    isMulti={false}
            ...
            <Grid container spacing={4}>
              ...
              <Grid item xs={12}>
                <span className={styles.description}>Expected salary. Please be notified that the VAT will be excluded from the amount.</span>
              </Grid>
              <Grid item container direction="row" spacing={2}>
                <Grid item xs={9} sm={6}>
                  <TextInput handleChange={e => setPriceFunction(e.target.value as unknown as number)} errors={[]} id='price'
                    value={price as unknown as string} type="number" placeholder="Salary" />
                </Grid>
                <Grid item xs={3} sm={6}>
                  <h5>€/day</h5>
                </Grid>
