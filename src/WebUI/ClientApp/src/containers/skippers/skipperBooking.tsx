...
const SkipperBooking: React.FC<IProps> = (props: IProps) => {
  const [
    currentlySelectedSkipperStatus,
    setCurrentlySelectedSkipperStatus,
  ] = useState<SkipperStatus>(SkipperStatus.Pending);
  const [initialPendingSkippers, setInitialPendingSkippers] = useState<
    Booking[]
  >([]);
  const [initialAcceptedSkippers, setInitialAcceptedSkippers] = useState<
    Booking[]
  >([]);
  ...
  const doSkipperAction = async (id: number, action: SkipperActionEnum) => {
    ...
    try {
      switch (action) {
        case SkipperActionEnum.Accept:
          await BookingApi.skipperAcceptBooking(id);
          break;
        case SkipperActionEnum.Decline:
          await BookingApi.skipperDeclineBooking(id);
          break;
        default:
          throw new Error("Unknown SkipperActionEnum " + action);
      }
      notificationContext.setSnackbar({
        showSnackbar: true,
  ...
  const updateBookingSkipperStatus = async (
  ...
  return (
    <div className={styles.wrapper}>
      <div className={styles.pitchLine} style={{ marginBottom: 30 }}>
        Hey, there are Guests interested in Sailing with you! Respond to their
        requests.
      </div>
      <Grid container item xs={12} alignItems="stretch">
        <Grid item container xs={12} direction="column">
          <SkippersHeader
            showDeclined={false}
            color="#23395b"
            updateSkipperStatus={updateBookingSkipperStatus}
          ></SkippersHeader>
            ...
            {bookingsToRender.length > 0 ? (
                ...
                <Grid item key={i} xs={12} className={styles.bookingCard}>
                  <BookingCard
                    ...
                    textForDisplay={
                      booking.guestName +
                      " (" +
                      booking.guestNationality!.label +
                      ")"
                    }
                    booking={booking}
                    bookingAction={doSkipperAction}
                  ></BookingCard>
                </Grid>
              ))
            ) : (
              <>
                ...
                {currentlySelectedSkipperStatus === SkipperStatus.Pending ? (
                  <Grid item xs={12} container>
                    <Grid item container xs={12}>
                      <Grid item md={9} xs={12}>
                        <p>
                          No booking requests? Update your availability in order
                          to get more chance for bookings requests!
                        </p>
                      </Grid>
                      <Grid item md={3} xs={12}>
                        <button
                          ...
                          onClick={() =>
                            props.history.push(CLIENT.SKIPPER.AVAILABILITY)
                          }
                        >
                  ...
                  <Grid>
                    <div>There are no requests to show.</div>
                  </Grid>
                )}
              </>
            )}
